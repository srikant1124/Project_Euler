...
class MatrixTests: XCTestCase {
    ...
    func creatMatrixWay2() {
        ...
        do {
            ...
            XCTAssertNotNil(matrixString)
        } catch MatrixError.InvalidRowAnColumn(let description) {
            print(description)
            XCTAssertNil(matrixString)
        } catch let erorr {
            print(erorr.localizedDescription)
        }
    }
    
    func testInputForHackerearth() {
        let sizeOfNetwork = readLine()
        if let size = sizeOfNetwork {
            print(size)
        }
    }
}
